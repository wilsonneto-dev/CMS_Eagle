...
// AcomodacaoFoto::_atualizar_ordem( 13, 10 );
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ){
	...
	if( isset( $_POST["action"] ) ){
		...
		if($_POST["action"] == "save_list_command" ){
			...
			if($_POST["order"] != ""){
				...
				while ($indice_ordem < count($arr_ordem) ) {
					...
					AcomodacaoFoto::_atualizar_ordem( $id, ( $indice_ordem + 1 ) );
			...
			while ( $indice < $quantidade_total ){
				...
				AcomodacaoFoto::_atualizar_descricao(
			...
			LogAdmin::_salvar( "Fotos da foto acomodacao atualizadas", "FotoAcomodacao" , $admin->id, "", "" );
		...
		while( $contador < count($_FILES["imagem"]["name"] ) ){
			...
			$foto->cod_acomodacao = $_GET["id"];
			if ( $_FILES["imagem"]["error"][$contador] == 0 ){
				$img_name = $p->id_url ."-". gerar_hash(12);
				$foto->imagem = Upload::salvaArqs( "acomodacoes/" . $img_name, $_FILES["imagem"], $contador );
			...
			$foto->ordem = 0;
			if ( $foto->cadastrar() ){
				LogAdmin::_salvar( "Foto acomodacao cadastrada", "FotoAcomodacao" , $admin->id, "", json_encode( $foto ) );
				$cont_ok++;
			} else {
				LogAdmin::_salvar( "Erro ao editar banner topo", "FotoAcomodacao" , $admin->id, "", json_encode( $foto ) );
			}
...
$page->html_content = StdGalleryManager::make(
	"SELECT 
		id, 
		descricao as texto, 
		cod_acomodacao as cod_referencia, imagem_thumb as imagem, 
		ordem FROM acomodacao_foto WHERE ativo = 1 and cod_acomodacao = '".str_replace( "'", "", intval( $p->id ) )."' order by ordem asc;",
	"Acomodacao"
);
