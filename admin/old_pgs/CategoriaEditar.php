...
if( isset( $_GET["id"] ) ){
	$p->id = $_GET[ "id" ];
	if($p->get()){
		if( $_SERVER['REQUEST_METHOD'] == 'POST' && $permissoes_admin[ $verificar_permissao ] == 1 ){
			...
			if( $p->cadastrar() ){
				LogAdmin::_salvar( "\"$p->titulo\" editado", "Categoria" , $admin->id , json_encode( $clone ), json_encode( $p ) );
				addOnloadScript("message('Atualizado com sucesso.','sucess');");
			} else {
				LogAdmin::_salvar( "Erro ao atualizr", "Categoria" , $admin->id , json_encode( $clone ), json_encode( $p ) );
