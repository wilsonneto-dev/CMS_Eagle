...
// ServicoFoto::_atualizar_ordem( 13, 10 );
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ){
	...
	if( isset( $_POST["action"] ) ){
		...
		if($_POST["action"] == "save_list_command" ){
			...
			if($_POST["order"] != ""){
				...
				while ($indice_ordem < count($arr_ordem) ) {
					...
					ServicoFoto::_atualizar_ordem( $id, ( $indice_ordem + 1 ) );
			...
			while ( $indice < $quantidade_total ){
				...
				ServicoFoto::_atualizar_descricao(
			...
			LogAdmin::_salvar( "Fotos da foto servico atualizadas", "FotoServico" , $admin->id, "", "" );
		...
		while( $contador < count($_FILES["imagem"]["name"] ) ){
			...
			$foto->cod_servico = $_GET["id"];
			if ( $_FILES["imagem"]["error"][$contador] == 0 ){
				$img_name = $p->id_url ."-". gerar_hash(12);
				$foto->imagem = Upload::salvaArqs( "servicos/" . $img_name, $_FILES["imagem"], $contador );
			...
			$foto->ordem = 0;
			if ( $foto->cadastrar() ){
				LogAdmin::_salvar( "Foto servico cadastrada", "FotoServico" , $admin->id, "", json_encode( $foto ) );
				$cont_ok++;
			} else {
				LogAdmin::_salvar( "Erro ao editar banner topo", "FotoServico" , $admin->id, "", json_encode( $foto ) );
			}
...
$page->html_content = StdGalleryManager::make(
	"SELECT 
		id, 
		descricao as texto, 
		cod_servico as cod_referencia, imagem_thumb as imagem, 
		ordem FROM servico_foto WHERE ativo = 1 and cod_servico = '".str_replace( "'", "", intval( $p->id ) )."' order by ordem asc;",
	"Servico"
);
