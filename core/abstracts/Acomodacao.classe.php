...
class Acomodacao{
	...
	public static function getTotal(){
		$_bd = new AcomodacaoDAO();
		return $_bd->getTotal();
	}
}

class AcomodacaoDAO extends BaseDAO{
	
	public function cadastra(Acomodacao $p){
		try {
			if($this->abreConexao()){
				$str_q = "INSERT INTO 
						acomodacao( 
	...
	public function getProxId(Acomodacao $p){
		try {
			if($this->abreConexao()){
				$str_q = "SELECT (MAX(id) + 1) AS proxId FROM acomodacao;";					
				if($q = $this->con->query($str_q)){
					if(!$q->num_rows == 0){
						$obj = $q->fetch_object();
						$p->id = $obj->proxId;
	...
	public function deleta(Acomodacao $obj){
		try {
			if($this->abreConexao()){
				$str_q = "UPDATE acomodacao
						SET ativo = 0
	...
	public function getTotal(){
		try {
			if($this->abreConexao()){
				$str_q = "SELECT COUNT(*) AS total FROM acomodacao WHERE codprojeto = ".CODPROJETO." AND ativo = 1;";					
				if($q = $this->con->query($str_q)){
					if(!$q->num_rows == 0){
