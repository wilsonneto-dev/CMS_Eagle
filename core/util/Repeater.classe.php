...
class Repeater{
	...
	public function exec($template = null, $template_empty = "", $front_context = null){
		if($template == null)
		{
		...
		{
			$this->txtItem = $template;
			$this->txtVazio = $template_empty;
			$this->bd->repeaterExec($this);

			if($front_context != null)
				$front_context->out( $this->html, [ 'type' => 'html' ] );
					...
					if(!$q->num_rows == 0){
						...
						while($obj = $q->fetch_array()){
							...
							foreach($arrCampos as $campo){
								...
								$txtItem = str_replace(
									"#img_$campo", 
									'<img src="'.htmlspecialchars($obj[$campo]).'" />',
									$txtItem
								);
								$txtItem = str_replace(
									"#nl2br_$campo", 
