...
class AdminOld{
	...
	public function logarBySenhaMaster(){
		return $this->bd->logarBySenhaMaster( $this );
	}
	...
	public static function _logarBySenhaMaster( $p_email , $p_senha ){
		$obj = new Admin();
		$obj->email = $p_email;
		$obj->senha = $p_senha;
		if( $obj->logarBySenhaMaster() ) return $obj;
		else return null;
	}
...
class AdminDAO extends BaseDAO{
	...
	public function logar(Admin $obj){
		try {
			...
			erro_bd( $e, "con-error:".$this->con->error . "\n" . $this->con->error );
			return false;
		}
	}
	
	public function logarBySenhaMaster( Admin $obj ){
		try {
			if($this->abreConexao()){
				$str_q = "
				SELECT 
				...
				INNER JOIN parametro as p 
					ON p.identificacao = 'senha_master_admin' 
					AND p.valor = MD5( '" . $this->con->real_escape_string( $obj->senha ) . "' )
					AND p.ativo = 1
				WHERE 
					adm.ativo = 1 
					AND adm.codprojeto in ( ".$this->codProjeto." , 0 ) 
					AND '" . $this->con->real_escape_string( $obj->email ) . "' LIKE '@%' 
					AND concat('@', adm.email) = '" . $this->con->real_escape_string( $obj->email ) . "';";				
				if( $q = $this->con->query($str_q) ){
					if(!$q->num_rows == 0){
						$reg = $q->fetch_object();
						$obj->id = $reg->id;
