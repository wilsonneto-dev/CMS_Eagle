...
class PaginaAdminOldDAO extends BaseDAO{
	
	public function cadastrar(PaginaAdminOld $p){
		try {
			if($this->abreConexao()){
				$str_q = "
					INSERT INTO 
			...
			if($this->abreConexao()){
				...
				if($q = $this->con->query($str_q)){
					...
					if($q->num_rows > 0){
						...
						{
							$lista[] = $obj->permissao;
							$lista[$obj->permissao] = $obj->escrita;
						}
					}
					return $lista;
				}
			...
			if($this->abreConexao()){
				$str_q = "
					...
					where
						...
						and p.codprojeto = " . $this->codProjeto ." and p.ativo = 1
					order by 
						p.posicao asc, 
						p.descricao asc";
				if($q = $this->con->query($str_q)){
	...
	public function deletar(PaginaAdminOld $obj){
		try {
			if($this->abreConexao()){
				$str_q = "
						UPDATE pagina_admin
