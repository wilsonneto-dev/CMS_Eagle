...
class FrontEnd 
{
	...
	{
		$theme_path = '/frontend/theme/' . $this->theme_path . '/' . $resource;
		if($out){
			$this->out($theme_path);
		}else{
			return $theme_path;
	...
	public function __construct(Info $_infos)
	{
		$this->infos = $_infos;
		$this->page = null;

		$this->theme_path = $this->infos->config_theme_path;
		$this->action_path = $this->infos->config_action_path;
	...
	public static function _render(Info $_infos = null ){
	    ...
	    $FrontEnd = new FrontEnd($_infos);
    	
    	if($_infos->carregar_paginas_info == 1)
		{
			$FrontEnd->page = Pagina::get([ 'url' => $FrontEnd->page_name ]);
			
			if($FrontEnd->page != null)
			{
				if($FrontEnd->page->header_titulo != '')
					$FrontEnd->set_title($FrontEnd->page->header_titulo);
				
				if($FrontEnd->page->header_descricao != '')
					$FrontEnd->set_description($FrontEnd->page->header_descricao);
				
				if($FrontEnd->page->header_palavras_chave != '')
					$FrontEnd->set_keywords($FrontEnd->page->header_palavras_chave);
			}
	...
	public function render ()
	{
		...
		if( file_exists("frontend/action/$this->action_path/index.master.cod.php") )
			include("frontend/action//$this->action_path/index.master.cod.php");

		if( file_exists("frontend/action/$this->action_path/$this->page_name.cod.php") )
			include("frontend/action/$this->action_path/$this->page_name.cod.php");
		else 
		{
			/* verificar no pagina layout qual trazer aqui */
			if( file_exists( "frontend/action//$this->action_path/pagina.cod.php" ) )
				include("frontend/action//$this->action_path/pagina.cod.php");
			else if( file_exists( "frontend/theme/$this->theme_path/views/404.pg.php" ) )
			{
				$this->redirect('/404');
			}
		}
		...
		if($this->page_name == 'landing' || $this->show_master == false)
		{
			$this->get_page_view();
		}
		else
		{
			if( file_exists("frontend/theme/$this->theme_path/views/index.master.php") )
				include("frontend/theme/$this->theme_path/views/index.master.php");			
		}
...
';
		...
		foreach ($this->_header_scripts as $script ) {
			if( strpos($script, "http") === false ){
		        $metas .= "\n".'		<script src="/frontend/theme/'.$this->theme_path.'/assets/'.$script.'"></script>';
			}else{
		...
		foreach ($this->_header_styles as $style ) {
		     $metas .= "\n".'		<link href="/frontend/theme/'.$this->theme_path.'/assets/'.$style.'" rel="stylesheet">';			
		}
	...
	public function get_page_view()
	{
		...
		if( file_exists("frontend/theme/$this->theme_path/views/$this->page_name.pg.php") )
			include("frontend/theme/$this->theme_path/views/$this->page_name.pg.php");
		else {
			/* verificar no pagina layout qual trazer aqui */
			if( file_exists( "frontend/theme/$this->theme_path/views/pagina.pg.php" ) )
				include("frontend/theme/$this->theme_path/views/pagina.pg.php");
			else
			{
	...
	{
		if( file_exists("frontend/theme/$this->theme_path/views/blocs/$bloc_name.bloc.php") )
			include("frontend/theme/$this->theme_path/views/blocs/$bloc_name.bloc.php");
	}

	public function bloc_cod($bloc_name)
	{
		if( file_exists("frontend/action/$this->action_path/blocs/$bloc_name.bloc.php") )
			include("frontend/action/$this->action_path/blocs/$bloc_name.bloc.php");
	}
