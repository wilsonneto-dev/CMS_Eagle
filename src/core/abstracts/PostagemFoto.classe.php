...
class PostagemFotoDAO extends BaseDAO {
	
	//CRUD
	public function cadastrar( PostagemFoto $obj ){
		try {
			if( $this->abreConexao() ){
				$str_q = "
					INSERT INTO 
						postagem_foto( 
							...
							,'". $this->con->real_escape_string( $obj->cod_postagem ) ."'
							, ". $this->codProjeto.
						");";
			...
			if($this->abreConexao()){
				...
				if( $q = $this->con->query( $str_q ) )
				{
					if( !$q->num_rows == 0 ){
						...
						$obj->cod_postagem = $reg->cod_postagem;
						$obj->ordem = $reg->ordem;
						return true;
					}
					else return false;	
			...
			if($this->abreConexao()){
				$str_q = "
					...
					SET 
						...
						, cod_postagem = '". $this->con->real_escape_string( $obj->cod_postagem ) ."'
	...
	public function deletar( PostagemFoto $obj ){
		try {
			if($this->abreConexao()){
				$str_q = "
					UPDATE 
